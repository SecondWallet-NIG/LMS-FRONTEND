    ...
    <div>
      <div className="p-3 bg-gray-100 text-base rounded-xl">
        <div className="flex gap-2 text-red items-center">
          <div className="text-sm text-swGray font-semibold">{title}</div>
        </div>
        <div className="flex items-center justify-between font-semibold mt-5">
          <p className="text-xl text-swBlue">
            <span>
              {title === "Total Loan Amount" || title === "Total Commitment Fee"
                ? "₦ "
                : null}
            </span>
            {value}
...
// }
const BorrowerSummaryCard = ({ data }) => {
  return (
    <main className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <Card
        title="Total Loan Amount"
        value={data?.totalLoanAmount?.toLocaleString()}
      />
      <Card title="Total Loan Count" value={data?.count} />
      <Card
        title="Total Commitment Fee"
        value={data?.totalCommitmentTotal?.toLocaleString()}
      />
