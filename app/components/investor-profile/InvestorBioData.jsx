...
export default function InvestorBioData({
    handleInfoHoverIn, data, labelClass, editButton,
    valueClass, dataClass, detailsHeader
}) {
    const pageDetailsData = [
        { label: <><LuCalendar size={16} />DOB</>, value: <>{data?.data?.dateOfBirth && format(new Date(data?.data?.dateOfBirth), "PPP")}</> },
        { label: <><FiSmile size={16} /> Gender</>, value: data?.data?.gender },
        { label: <><FiHash size={16} /> NIN</>, value: data?.data?.nin },
        { label: <><FiPhone size={16} /> Phone</>, value: data?.data?.phoneNumber },
        { label: <><FiMail size={16} /> Email</>, value: data?.data?.email }
    ]
    ...
    return (
        <div>
            ...
            <div className="p-2 border-t-2 border-gray-300">
                ...
                <div className="mt-2">
                    {pageAddressData.map((d, index) => <div key={index}>{renderPageData(d.label, d.value)}</div>)}
                </div>
            </div>
            ...
            <div className="p-2 border-t-2 border-gray-300 lg:mb-20">
                ...
                <div className="">
                    {pageBankData.map((d, index) => <div key={index}>{renderPageData(d.label, d.value)}</div>)}
                </div>
            </div>
        </div>
    )
